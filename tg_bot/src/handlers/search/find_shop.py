...
async def handle_location(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(
            'Выберите магазин:',
            reply_markup=create_shop_buttons(data, 0, longitude, latitude, initial_shop_url)
        )
...
async def prepare_shop_caption(shop: dict, count_products: int) -> str:
    return 'Название магазина: %s\nОписание: %s\n%s' %(
        ...
        'Продукты для этого магазина:' if count_products else 'Продукты для этого магазина отсутствуют'
    )
...
async def handle_shop_callback(callback_query: CallbackQuery, callback_data: ShopCallback, state: FSMContext):
    try:
        ...
        await callback_query.message.answer_photo(
            photo=input_file,
            caption=caption,
            reply_markup=create_product_buttons(products, shop, offset=0, back_url=back_url)
        )
        await state.set_state(SearchState.find_product)
    except ClientResponseError as e:
        await callback_query.message.answer('Произошла ошибка при поиске магазинов. {}'.format(e))
...
async def handle_pagination(callback_query: types.CallbackQuery, callback_data: PaginationCallback, state: FSMContext):
    offset = int(callback_data.offset)
    shop_url = await prepare_shop_from_callback(
        callback_data.back_url,
    ...
    try:
        ...
        await callback_query.message.edit_reply_markup(
            reply_markup=create_shop_buttons(
                ...
                callback_data.latitude,
                callback_data.back_url
            ) if callback_data.source == 'shop' else create_product_buttons(
                shops.get('products'),
                shops.get('shop'),
                offset=offset,
        ...
        await callback_query.message.answer('Произошла ошибка при обновлении списка магазинов.')